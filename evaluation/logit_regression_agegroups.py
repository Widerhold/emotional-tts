...
for pid, row in df_raw.iterrows():
    age_label = age_map.get(row["Altersgruppe"])
...
for emotion in emotion_groups:
    for sys in voice_labels.values():
        ...
        # überspringen, wenn nur eine Altersgruppe vertreten
        if sub["Altersgruppe"].nunique() < 2:
            skip_counter += 1
            continue
        ...
        for pname in [p for p in gee_res.params.index
                      if p.startswith("C(Altersgruppe)[T.")]:
...
if not res_df.empty:
    ...
    print(f"Kombinationen übersprungen (nur eine Altersgruppe präsent): {skip_counter}\n")

    sig = res_df[res_df["p_adj"] < 0.05]
    if sig.empty:
        print("Keine Holm-signifikanten Altersgruppeneffekte gefunden.")
    else:
        print("Signifikante Altersgruppeneffekte (Holm-adjustiert p < 0.05):")
